...
void LteRealisticChannelModel::initialize()
{
   ...
   FinalrcvdPower_=registerSignal("FinalrcvdPower");
}


double LteRealisticChannelModel::getAttenuation(MacNodeId nodeId, MacNodeId eNbId, Direction dir, Coord coord)//Masoud
{
   double movement = .0;
   double speed = .0;
...
//       speed = computeSpeed(nodeId, coord);
   ...
   double dbp = 0;
   double attenuation = computePathLoss(sqrDistance, eNbId, dbp, los);//Masoud
...
double LteRealisticChannelModel::getAttenuation_D2D(MacNodeId nodeId, Direction dir, Coord coord,MacNodeId node2_Id, Coord coord_2)
{
   ...
   bool los = losMap_[nodeId];
   double dbp = 0;
   double attenuation = 0;//computePathLoss(sqrDistance, dbp, los);//Masoud: I don't need it
...
std::vector<double> LteRealisticChannelModel::getSINR(LteAirFrame *frame, UserControlInfo* lteInfo)
{
   ...
   if ((lteInfo->getFrameType() == FEEDBACKPKT))
       attenuation = getAttenuation(ueId,eNbId, UL, coord); // dB//Masoud
   else
       attenuation = getAttenuation(ueId,eNbId, dir, coord); // dB//Masoud
   ...
   speed=0;
   double finalRecvPower2;
   double finalRecvPower;
   ...
   for (unsigned int i = 0; i < band_; i++)
   {
       ...
       finalRecvPower = recvPower + fadingAttenuation; // (dBm+dB)=dBm
       finalRecvPower2 = recvPower + fadingAttenuation; // (dBm+dB)=dBm  //////////Masoud
   ...
   {
       finalRecvPower2 -= den;
       emit (FinalrcvdPower_, finalRecvPower2);
   }
...
double LteRealisticChannelModel::computePathLoss(double distance, MacNodeId eNbId, double dbp, bool los)//Masoud
{
    //compute attenuation based on selected scenario and based on LOS or NLOS
    double pathLoss = 0;
    switch (scenario_)
    ...
    case URBAN_MICROCELL:
        ...
        break;
    case URBAN_MACROCELL:
        pathLoss = computeUrbanMacro(distance,eNbId, los);//Masoud
        break;
    case RURAL_MACROCELL:
...
double LteRealisticChannelModel::computeUrbanMacro(double d,MacNodeId eNbId, bool los) //Masoud
{
   if (d < 10)
       d = 10;

///////////////////////////////////////masoud////////////////////////////////////////////////////
   switch (eNbId)// ||eNbId == 2 ||eNbId == 3 ||eNbId == 4)
   {
   case 1:
       hNodeB_= 25;
       break;
       ...
       std::vector<double>* interference)
{
   ...
   while (it != list.end())
   {
       ...
       att = computeExtCellPathLoss(dist, nodeId, eNbId);// Masoud
...
double LteRealisticChannelModel::computeExtCellPathLoss(double dist, MacNodeId nodeId, MacNodeId eNbId)
{
   double movement = .0;
   double speed = .0;
   ...
   bool los = losMap_[nodeId];
   double dbp = 0;
   double attenuation = computePathLoss(dist,eNbId, dbp, los);//Masoud
       ...
       std::vector<double> * interference)
{
   ...
   while(it!=et)
   {
       ...
       att = (*it)->realChan->getAttenuation(ueId,eNbId,UL,coord);//Masoud
   ...
   if(isCqi)// check slot occupation for this TTI
   {
       for(unsigned int i=0;i<band_;i++)
       {
           ...
           for (; ue_it != ue_et; ++ue_it)
           {
               ...
               double att = getAttenuation(ueId,eNbId, UL, uePhy->getCoord());//Masoud
               (*interference)[i] += dBmToLinear(txPwr-att);//(dBm-dB)=dBm
